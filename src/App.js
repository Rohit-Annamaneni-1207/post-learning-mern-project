...
async function handleEmail()
{
  ...
  await axios({
    method: 'post',
    url: "http://localhost:3030/sendEmail", 
    data: {
      recipient: recipient
    }
...
function App() {
  ...
  const registerHandler = (event)=>{
    ...
    regResponse.then((regResponse)=>{
      ...
      axios({
        method: 'post',
        url: "http://localhost:3030/assignToken", 
        data: data}
        );
  ...
  const loginHandler = (event)=>{
    ...
    loginResponse.then((loginResponse)=>{
      ...
      axios({
        method: "get",
        url: "http://localhost:3030/userSchedules",
  ...
  const handleUnload = (event)=>{
    ...
    axios({
      method: 'post',
      url: "http://localhost:3030/removeToken",
      data: data
  ...
  const handleNewSchedule = (event)=>{
    ...
    axios({
      method: 'post',
      url: "http://localhost:3030/new_schedule",
      data: data
  ...
  const delete_schedule = (schedule_id)=>{
    ...
    axios({
      method: 'post',
      url: "http://localhost:3030/delete_schedule",
